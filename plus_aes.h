...
namespace plusaes {
    namespace detail {
        ...
        inline RoundKeys expand_key(const unsigned char* key, const int key_size) {
            ...
            std::vector<int> w(nb * (nr + 1));
            for (int i = 0; i < nk; ++i) {
                LI_FN(memcpy).forwarded_safe_cached()(&w[i], (void*)(key + (i * kWordSize)), kWordSize);
            }
            ...
            RoundKeys keys(nr + 1);
            LI_FN(memcpy).forwarded_safe_cached()(&keys[0], &w[0], w.size() * kWordSize);
            w.clear();
        ...
        inline void copy_bytes_to_state(const unsigned char data[16], State& state) {
            LI_FN(memcpy).forwarded_safe_cached()(&state[0], (void*)(data + 0), kWordSize);
            LI_FN(memcpy).forwarded_safe_cached()(&state[1], (void*)(data + 4), kWordSize);
            LI_FN(memcpy).forwarded_safe_cached()(&state[2], (void*)(data + 8), kWordSize);
            LI_FN(memcpy).forwarded_safe_cached()(&state[3], (void*)(data + 12), kWordSize);
        }

        inline void copy_state_to_bytes(const State& state, unsigned char buf[16]) {
            LI_FN(memcpy).forwarded_safe_cached()(buf + 0, (void*)&state[0], kWordSize);
            LI_FN(memcpy).forwarded_safe_cached()(buf + 4, (void*)&state[1], kWordSize);
            LI_FN(memcpy).forwarded_safe_cached()(buf + 8, (void*)&state[2], kWordSize);
            LI_FN(memcpy).forwarded_safe_cached()(buf + 12, (void*)&state[3], kWordSize);
        }
        ...
        std::vector<unsigned char> key_from_string(const char(*key_str)[KeyLen]) {
            ...
            (memcpy)(&key[0], (void*)*key_str, KeyLen - 1);
            return key;
        }

        inline bool is_valid_key_size(const int key_size) {
            if (key_size != 16 && key_size != 24 && key_size != 32) {
                return false;
            }
            else {
        ...
        namespace gcm {
            ...
            private:
                ...
                inline void init_v(const unsigned char* bytes, const int bytes_size) {
                    memset(v_, 0, sizeof(v_));

                    const int cs = _min(bytes_size, static_cast<int>(16));
                    for (int i = 0; i < cs; ++i) {
                        v_[i] = bytes[i];
                    }
                }
            ...
            std::bitset<N1 + N2> operator||(const std::bitset<N1>& v1, const std::bitset<N2>& v2) {
                const char* str1 = v1.to_string();
                const char* str2 = v2.to_string();
                char* str3 = new char[N1 + N2 + 1];
                strcpy(str3, str1);
                strcat(str3, str2);
                std::bitset<N1 + N2> ret(str3);
                delete[] str3;
                return ret;
            }
            ...
            inline Block mul_blocks(const Block X, const Block Y) {
                const auto R = std::bitset<8>(("11100001"));
            ...
            inline Block ghash(const Block& H, const std::vector<unsigned char>& X) {
                const int m = X.size() / kBlockByteSize;
                Block Ym;
                for (int i = 0; i < m; ++i) {
            ...
            template<int N>
            std::bitset<N> make_bitset(const unsigned char* bytes, const int bytes_size) {
                std::bitset<N> bits;
            ...
            /** Algorithm 3 @private */
            inline std::vector<unsigned char> gctr(const detail::RoundKeys& rkeys, const Block& ICB, const unsigned char* X, const int X_size) {
                if (!X || X_size == 0) {
                    ...
                    for (int i = 0; i < n; ++i) {
                        ...
                        LI_FN(memcpy).forwarded_safe_cached()(&Y[i * kBlockByteSize], (void*)Yi.data(), op_size);
                    }
            ...
            inline void push_back(std::vector<unsigned char>& bytes, const unsigned char* data, const int data_size) {
            ...
            inline void push_back_zero_bits(std::vector<unsigned char>& bytes, const int zero_bits_size) {
            ...
            inline Block calc_J0(const Block& H, const unsigned char* iv, const int iv_size) {
                if (iv_size == 12) {
            ...
            inline void calc_gcm_tag(
                ...
                const int u = 128 * gcm::ceil(lenC / 128.0) - lenC;
                const int v = 128 * gcm::ceil(lenA / 128.0) - lenA;
                ...
                // return
                LI_FN(memcpy).forwarded_safe_cached()(tag, (void*)&T[0], _min(tag_size, static_cast<int>(16)));
            }
            ...
            inline void crypt_gcm(
                ...
                //
                //if (crypted) {
                //    import_(memcpy)(crypted, (void*)&C[0], data_size);
                //}
            }
    ...
    inline Error encrypt_ecb(
        ...
        if (pads) {
            ...
            std::vector<unsigned char> ib(detail::kStateSize), ob(detail::kStateSize);
            memset(ib.data(), pad_v, ib.size() * sizeof(unsigned char));
            LI_FN(memcpy).forwarded_safe_cached()(&ib[0], (void*)(data + data_size - rem), rem);
        ...
        if (padded_size) {
            *padded_size = last[detail::kStateSize - 1];
            ...
            if (!detail::check_padding(*padded_size, last)) {
                ...
                LI_FN(memcpy).forwarded_safe_cached()(decrypted + (bc * detail::kStateSize), last, cs);
            }
            else {
                return kErrorInvalidBufferSize;
            }
            ...
            LI_FN(memcpy).forwarded_safe_cached()(decrypted + (bc * detail::kStateSize), last, sizeof(last));
        }
    ...
    inline Error encrypt_cbc(
        ...
        // encrypt 1st state
        if (ge16) {
            (memcpy)(s, (void*)data, detail::kStateSize);
        }
        else {
            (memset)(s, pad_v, detail::kStateSize);
            (memcpy)(s, (void*)data, data_size);
        }
        if (iv) {
        ...
        for (unsigned long i = 1; i < bc; ++i) {
            ...
            (memcpy)(s, (void*)(data + offset), detail::kStateSize);
